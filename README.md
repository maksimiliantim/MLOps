...
### 3.1 Расчёт среднего и пикового RPS

**Дано:**
- DAU = **3 858 438** пользователей/день.
- Примем среднее число поисковых запросов: **25 запросов/пользователь/день** (допустим).
...
Чтобы оценить **пиковый RPS**, вводим коэффициент пиковости.
Если принять peak factor ~ **10.8×**, то:
- `RPS_peak ≈ 1 116 × 10.8 ≈ 12 053 запросов/сек`

Итого, пиковая нагрузка **~12k запросов/сек**.
...
**Объём/день:**
- search_events: `96.5M × 3 KB ≈ 290 GB/day`
- impressions: `96.5M × 20 × 0.5 KB ≈ 920 GB/day`
- clicks: `96.5M × 2 × 0.5 KB ≈ 92 GB/day`
- orders: `96.5M × 0.1 × 1 KB ≈ 9.2 GB/day`
- сумма: `290 + 920 + 92 + 9.2 ≈ 1 311 GB/day ≈ 1.31 TB/day`

**Итого:** ~**1.30 TB/day** (без учёта компрессии, служебных полей и ретраев).

**За 30 дней:** `1.31 × 30 ≈ 40 TB`

С запасом на ретраи, дополнительные поля, metadata Iceberg и промежуточные витрины обычно **~1.5–2×**:
